...
CREATE TABLE `AGENTS` (
  ...
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='Agent 테이블\n전체 agent 정보를 관리';
...
CREATE TABLE `AGENT_GROUPS` (
  ...
  `ZONE` varchar(45) NOT NULL COMMENT '플랫폼 내의 환경(dev/stg/prd 등)',
  `PLATFORM` varchar(45) NOT NULL COMMENT '플랫폼(baremetal, k8s, aws 등)',
  `CREATED_AT` timestamp NULL DEFAULT NULL,
  `UPDATED_AT` timestamp NULL DEFAULT NULL,
  `DELETED_AT` timestamp NULL DEFAULT NULL,
  PRIMARY KEY (`ID`),
  UNIQUE KEY `UNQ_01` (`USER_ID`,`PLATFORM`,`ZONE`,`GROUP_NAME`),
  KEY `IDX_01` (`USER_ID`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='agent 그룹 관리 테이블';
/*!40101 SET character_set_client = @saved_cs_client */;
...
-- Dump completed on 2020-07-20 13:01:38
